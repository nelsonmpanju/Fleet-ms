	...
	data = frappe.db.sql('''
		SELECT 
			...
			(
				...
				WHERE parenttype = 'Trips' AND parent = `tabTrips`.name AND request_currency = 'USD' AND request_status = 'Approved' AND ledger_entry != ''
			) AS trip_expenses_usd,
			(
				SELECT SUM(request_amount)
				FROM `tabRequested Fund Details`
				WHERE parenttype = 'Trips' AND parent = `tabTrips`.name AND request_currency = 'TZS' AND request_status = 'Approved' AND ledger_entry != ''
