...
def approve_request(**args):
    ...
    # Auto-process: create stock entry or purchase invoice on the Trip
    if doc.parenttype == "Trips":
        try:
            from vsd_fleet_ms.vsd_fleet_ms.doctype.trips.trips import auto_process_fuel_approval
            auto_process_fuel_approval(doc.name)
        except Exception as e:
            frappe.log_error(f"Auto fuel processing failed for {doc.name}: {str(e)}", "Fuel Approval")
