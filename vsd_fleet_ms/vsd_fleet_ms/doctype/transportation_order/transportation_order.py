...
def create_sales_invoice(doc, rows):
    ...
    if not selected_names:
        frappe.throw(_("Please select at least one transport row to invoice."))

    selected_rows = [row for row in doc.assign_transport if row.name in selected_names]
    if not selected_rows:
        frappe.throw(_("Selected transport rows were not found in this document."))
    ...
    for row in selected_rows:
        if not row.item:
            frappe.throw(_("Item is required for row {0}.").format(row.idx))
        ...
        if row.transporter_type == "In House":
            if row.assigned_vehicle:
                description += "<b>VEHICLE NUMBER: {0}</b>".format(cstr(row.assigned_vehicle))
            if row.created_trip:
            ...
            description += "<b>VEHICLE NUMBER: {0}</b>".format(cstr(row.vehicle_plate_number))

        if row.route:
            description += "<br>ROUTE: {0}".format(cstr(row.route))
    ...
    for row in selected_rows:
        ...
        if row.transporter_type == "In House" and row.created_trip:
            trip = frappe.get_doc("Vehicle Trips", row.created_trip)
